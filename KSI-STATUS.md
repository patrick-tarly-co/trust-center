# Tarly Cowork — FedRAMP 20x KSI Status

FedRAMP package ID: **FR2628650874**

Generated 2026-08-15T08:30:31.804419+00:00 by the nightly compliance pipeline. Machine-readable version: [ksi-results.json](ksi-results.json). Human-facing trust center: <https://gov.tarly.co/trust/>.

| Status | Count |
| --- | --- |
| Met | 18 |
| Partially met | 26 |
| Not met | 1 |
| Unknown | 1 |

| KSI | Name | Status | Target date |
| --- | --- | --- | --- |
| KSI-CED-RAT | Reviewing All Training | Met |  |
| KSI-CMT-LMC | Logging Changes | Partially met | None |
| KSI-CMT-RMV | Redeploying vs Modifying | Met |  |
| KSI-CMT-RVP | Reviewing Change Procedures | Met |  |
| KSI-CMT-VTD | Validating Throughout Deployment | Met |  |
| KSI-CNA-DFP | Defining Functionality and Privileges | Met |  |
| KSI-CNA-EIS | Enforcing Intended State | Unknown |  |
| KSI-CNA-IBP | Implementing Best Practices | Partially met | None |
| KSI-CNA-MAT | Minimizing Attack Surface | Partially met | None |
| KSI-CNA-OFA | Optimizing for Availability | Partially met | None |
| KSI-CNA-RNT | Restricting Network Traffic | Met |  |
| KSI-CNA-RVP | Reviewing Protections | Partially met | None |
| KSI-CNA-ULN | Using Logical Networking | Met |  |
| KSI-IAM-AAM | Automating Account Management | Partially met | None |
| KSI-IAM-APM | Adopting Passwordless Methods | Met |  |
| KSI-IAM-ELP | Ensuring Least Privilege | Partially met | None |
| KSI-IAM-JIT | Authorizing Just-in-Time | Partially met | None |
| KSI-IAM-SNU | Securing Non-User Authentication | Partially met | None |
| KSI-IAM-SUS | Responding to Suspicious Activity | Met |  |
| KSI-INR-AAR | Generating After Action Reports | Met |  |
| KSI-INR-RIR | Reviewing Incident Response Procedures | Met |  |
| KSI-INR-RPI | Reviewing Past Incidents | Met |  |
| KSI-MLA-ALA | Authorizing Log Access | Met |  |
| KSI-MLA-EVC | Evaluating Configurations | Met |  |
| KSI-MLA-LET | Logging Event Types | Partially met | None |
| KSI-MLA-OSM | Operating SIEM Capability | Partially met | None |
| KSI-MLA-RVL | Reviewing Logs | Partially met | None |
| KSI-PIY-GIV | Generating Inventories | Met |  |
| KSI-PIY-RES | Reviewing Executive Support | Partially met | None |
| KSI-PIY-RIS | Reviewing Investments in Security | Met |  |
| KSI-PIY-RSD | Reviewing Security in the SDLC | Met |  |
| KSI-PIY-RVD | Reviewing Vulnerability Disclosures | Partially met | None |
| KSI-RPL-ABO | Aligning Backups with Objectives | Partially met | None |
| KSI-RPL-ARP | Aligning Recovery Plan | Partially met | None |
| KSI-RPL-RRO | Reviewing Recovery Objectives | Partially met | None |
| KSI-RPL-TRC | Testing Recovery Capabilities | Partially met | None |
| KSI-SCR-MIT | Mitigating Supply Chain Risk | Partially met | None |
| KSI-SCR-MON | Monitoring Supply Chain Risk | Met |  |
| KSI-SVC-ACM | Automating Configuration Management | Partially met | None |
| KSI-SVC-ASM | Automating Secret Management | Partially met | None |
| KSI-SVC-EIS | Evaluating and Improving Security | Partially met | None |
| KSI-SVC-PRR | Preventing Residual Risk | Partially met | None |
| KSI-SVC-RUD | Removing Unwanted Data | Not met | None |
| KSI-SVC-SIN | Securing Information | Partially met | None |
| KSI-SVC-VCM | Validating Communications | Partially met | None |
| KSI-SVC-VRI | Validating Resource Integrity | Partially met | None |
